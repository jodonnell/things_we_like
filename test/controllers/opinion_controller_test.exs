defmodule ThingsWeLike.OpinionControllerTest do
  ...
  test "creates and renders resource when data is valid", %{conn: conn} do
    create_category
    conn = post conn, opinion_path(conn, :create), %{opinion: @valid_attrs, category: "Pow"}
  ...
  test "does not create resource and renders errors when data is invalid", %{conn: conn} do
    create_category
    conn = post conn, opinion_path(conn, :create), %{opinion: @invalid_attrs, category: "Pow"}
  ...
  defp create_category do
    changeset = Category.changeset(%Category{}, %{name: "Pow"})
